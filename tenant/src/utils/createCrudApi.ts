 ...
 *
 * export const roleApi = createCrudApi<RoleInfo, RoleReq>('/tenantapi/system/role')
 ...
 *   ...createCrudApi<RoleInfo, RoleReq>('/tenantapi/system/role'),
 *   getPermissions: (id: number) => myRequest.get(`/tenantapi/system/role/${id}/permissions`),
 * }
 * ```
 *
 * @template T 列表项/详情类型
 * @template Req 创建/更新请求体类型
 * @param basePath 资源根路径（不含结尾斜杠），如 `/tenantapi/system/role`
 */
