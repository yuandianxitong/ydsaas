...
import type { AdminInfo, AuthInfoRes, MenuInfo, RoleInfo, SaasInfo } from '@/types/api'
...
export const useUserStore = defineStore('user', () => {
    ...
    const saas = ref<SaasInfo | null>(null)
    ...
    const lifecycleState = computed(() => saas.value?.lifecycle_state || 'active')
    const isGrace = computed(() => lifecycleState.value === 'grace')
    const isFrozen = computed(() => lifecycleState.value === 'frozen')
    const isDisabled = computed(() => lifecycleState.value === 'disabled')
    ...
    // ---- Entitlement helpers（Phase 2 子系统 A 引入） ----

    function hasEntitlement(code: string): boolean {
        return (saas.value?.entitlements ?? []).some((e) => e.code === code)
    }

    function hasAnyEntitlement(codes: string[]): boolean {
        if (!codes || codes.length === 0) return true
        const list = saas.value?.entitlements ?? []
        return list.some((e) => codes.includes(e.code))
    }

    function getEntitlement(code: string): import('@/types/api').Entitlement | null {
        return (saas.value?.entitlements ?? []).find((e) => e.code === code) ?? null
    }

    /** @deprecated 改用 hasEntitlement(code) */
    function hasFeature(code: string): boolean {
        return hasEntitlement(code)
    }

    /** @deprecated 改用 hasAnyEntitlement(codes) */
    function hasAnyFeature(codes: string[]): boolean {
        return hasAnyEntitlement(codes)
    }
