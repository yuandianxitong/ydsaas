...
export function filterAsyncRoutes(routes: MenuInfo[]): RouteRecordRaw[] {
    return routes
        .map((route) => {
...
export function createRouteRecord(route: MenuInfo): RouteRecordRaw {
    // 调试：检查路由数据
    ...
    const routeRecord = {
        // 统一规整为绝对路径：目录子菜单(相对 path)不再嵌套到父级合成的 /dir-<id> 下，
        // 与租户端约定一致（子菜单始终绝对路径）
        path: isExternal(route.path) ? route.path : `/${route.path.replace(/^\//, '')}`,
        ...
        meta: {
            hidden: route.meta?.hidden ?? !!route.hidden,
            keepAlive: route.meta?.cache,
            title: route.meta?.title || route.title || '',
            perms: route.meta?.permission || route.permission || '',
