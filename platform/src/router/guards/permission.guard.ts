...
import { DASHBOARD_ROUTE, INDEX_ROUTE, INDEX_ROUTE_NAME } from '@/router/routes.config' // 从 routes.config 导出
...
export default function createPermissionGuard(router: Router): void {
    ...
    const whiteList: string[] = [
        ...
        PageEnum.ERROR_404,
        PageEnum.ERROR_500,
        // 官方市场 OAuth 回调弹窗：只读 query→postMessage→关闭，无需登录/菜单加载
        ...
        if (token) {
            ...
            if (hasUserInfo && userStore.isRoutesInited) {
                ...
                try {
                    ...
                    // 如果后端菜单为空（CLI 创建超管但没跑安装器），fallback 到内置 Dashboard
                    if (accessedRoutes.length === 0) {
                        accessedRoutes.push(DASHBOARD_ROUTE)
                    }

                    // 找到第一个有效页面的 path
                    const firstRoutePath =
                        findFirstValidPath(accessedRoutes) || (DASHBOARD_ROUTE.path as string)
