<template>
    <div class="version-container">
        <div class="page-head">
            <div>
                <div class="page-title">{{ $t('versionMgmt.title') }}</div>
                <div class="page-desc">{{ $t('versionMgmt.desc') }}</div>
            </div>
        ...
        <ProTable
            ...
            storage-key="platform-version-list"
            :columns="columns"
            :data="list"
            :loading="loading"
...
const {
    ...
    batchDeleteFn: (ids) => Promise.all(ids.map((id) => versionApi.delete(id))),
    defaultSearchForm: { platform: undefined, status: undefined }
})

const columns: ProColumn[] = [
    { key: 'id', label: t('common.id'), prop: 'id', width: 80, required: true },
    { key: 'platform', label: t('versionMgmt.platform'), prop: 'platform', width: 120 },
