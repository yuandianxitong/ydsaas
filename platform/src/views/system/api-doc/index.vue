...
// 默认展示「管理端」(平台 platformapi) 文档 —— 这是平台管理 UI，最相关
const apiType = ref('platform')

const getOpenapiUrl = () => {
    // 始终用相对路径，开发环境走 Vite 代理，生产环境走同域
    return `/platformapi/system/api-doc/openapi.json?type=${apiType.value}`
}
...
const openSwaggerUI = () => {
    window.open(`/platformapi/system/api-doc?type=${apiType.value}`, '_blank')
}
