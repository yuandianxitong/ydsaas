<template>
    <div class="file-page flex flex-col gap-4 h-full">
        <div class="page-head">
            <div>
        ...
        <div class="fm-layout">
            ...
            <aside class="fm-side">
                <div class="fm-side-t">{{ $t('file.category') }}</div>
                <div class="fm-side-list flex flex-col gap-0.5">
                    <button
                        v-for="t in typeOptions"
                        :key="t.value"
                        :class="['fm-side-item flex items-center gap-2.5 px-2.5 py-2 rounded-md', { on: currentType === t.value }]"
            ...
            <div class="fm-main">
                <!-- Toolbar -->
                <div class="fm-toolbar flex items-center justify-between gap-2.5">
                    <div class="fm-toolbar-l flex items-center gap-2.5 flex-wrap">
                        <template v-if="selectedIds.length > 0">
                            <label class="fm-chkall inline-flex items-center gap-2 py-1">
                                <input
                                    type="checkbox"
                        ...
                        <template v-else>
                            ...
                            <div class="fm-sep" />
                            <div class="fm-sort inline-flex items-center gap-0.5">
                    ...
                    <div class="fm-toolbar-r flex items-center gap-2">
                        <div class="fm-view">
                            <button
                ...
                <div v-else-if="fileList.length === 0" class="fm-empty flex-center flex-1 min-h-300px">
                            ...
                            >
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        ...
                                        <el-dropdown-item
                                            v-if="hasPerm('platform.file.update')"
                                            command="rename"
                                        >
                                        ...
                                        <el-dropdown-item
                                            v-if="hasPerm('platform.file.delete')"
                                            command="delete"
                                            divided
                                        >
                ...
                <div v-else class="fm-list">
                    ...
                    >
                        ...
                        <div style="flex: 1 1 320px; display: flex; align-items: center; gap: 10px; min-width: 0">
                            <span
                                class="fm-list-ext"
                                :style="{ background: extColor(file) }"
                            >{{ (file.extension || '').toUpperCase() }}</span>
...
// Type counts (computed from file list when possible, otherwise mock)
const typeCounts = computed(() => {
    const c: Record<string, number> = { '': total.value }
    for (const f of fileList.value) {
        const t = getFileType(f)
        c[t] = (c[t] || 0) + 1
    }
    // The total is the API total, not just current page
    return c
})
...
const typeOptions = [
    ...
    { value: 'misc', label: 'file.other', ...FILE_TYPE_COLORS.other }
]
...
const handleDelete = async (row: any) => {
    try {
        await ElMessageBox.confirm(t('file.deleteFileConfirm', { name: row.name }), t('common.tips'), {
            type: 'warning'
        })
    } catch {
        return
