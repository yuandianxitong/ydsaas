...
const REFRESH_URL = '/platformapi/auth/refresh'
...
request.interceptors.request.use(
    async (config: any) => {
        ...
        // 添加当前语言（Accept-Language header 供后端 LocaleMiddleware 解析）
        const locale = getLocale()
        if (config.headers) {
            config.headers['Accept-Language'] =
                locale === 'zh-CN' ? 'zh-CN,zh;q=0.9' : 'en-US,en;q=0.9'
        }
        ...
        if (error.response) {
            ...
            switch (status) {
                case 401:
                    ...
                    break
                // 刻意不抄 tenant 的 402 分支：平台端无租户生命周期（grace/frozen），
                // TenantStatusMiddleware 不会对 platformapi 返回 402；若偶发收到，走 default。
                case 403:
