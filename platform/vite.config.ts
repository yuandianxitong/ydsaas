...
export default defineConfig(({ mode }: ConfigEnv) => {
    ...
    return {
        ...
        server: {
            ...
            open: true,
            proxy: {
                // 代理 /platformapi 的请求到后端API
                '/platformapi': {
                    target: env.VITE_APP_API_URL || 'http://test.com',
                    changeOrigin: true,
                    secure: false,
                    rewrite: (path) => path.replace(/^\/platformapi/, '/platformapi'),
                    configure: (proxy) => {
                        proxy.on('error', (err) => {
        ...
        build: {
            // 构建产物输出到 server/public/platform/，部署时无需手动复制
            outDir: resolve(__dirname, '../server/public/platform'),
            emptyOutDir: true,
            // 拆包后单 chunk 仍可能偏大（element-plus）；800 便于发现回归，勿再抬到 2000 掩盖
