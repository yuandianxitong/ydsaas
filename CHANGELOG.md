...
### Changed
- 平台管理端视觉对齐 YDAdmin Pro / Shop：墨色图标栏、`#4f6bff` 代币栈、13px 密度与 Element Plus 覆盖；去掉暗色模式。侧栏间距、列表页头（标题+描述+右侧主按钮）、筛选卡与表格卡底部分页一并对齐。租户端未改
- 平台剩余列表页接入 ProTable（订单、退款、审计、版本、地区、移动端构建、权限、字典、定时任务、登录/操作日志）；有删除接口的页支持批量删除，只读页不加
- 平台管理端继续对齐 Shop：一级栏 8px 间距、ProTable/列配置、系统配置左导航、菜单左树右表单；文件/生成器/插件/授权/API 文档改用同一套 set/fm 卡壳。租户端未改
- 平台细节对齐 Shop：列表表体内边距、操作列删除红色、文件网格/列表与分类侧栏、字典项弹窗、日志 Tab 用配送记录分段样式并左右分布在筛选栏、弹窗仅 body 滚动。租户端未改
