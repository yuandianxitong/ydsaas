...
> 说明：本项目已彻底移除 `think-migration`。全新安装（web 安装向导与 `php think saas:install`）统一从
> `public/install/data/schema.sql` + `init.sql` 建库，不再执行 `php think migrate:run`。
> 插件（`plugins/*`）的建表/升级由插件系统自身的 `database/install.sql` 与 `database/updates/` 管理，与框架 `yd:update` 相互独立。
...
```bash
# 告诉升级器“我的库当前已是 2.31.0”，<= 2.31.0 的脚本全部记为已应用，只执行之后的版本
php think yd:update --baseline=2.31.0
...
**升级脚本一律书写裸表名**（如 `admins`、`tenants`），不要手写前缀。执行时升级器会用与安装程序完全一致的逻辑（`core/database/SqlRunner`）自动套上当前 `DB_PREFIX`。因此无论用户是否设置了表前缀，同一份脚本都能正确执行。
...
```php
<?php
// database/updates/v2.32.0/update.php
