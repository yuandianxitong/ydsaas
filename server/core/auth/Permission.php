...
class Permission
{
    ...
    {
        // 超级管理员（持有 is_system=1 角色）拥有所有权限。
        // 不再用硬编码 userId===1：多租户下每个租户都有自己的超管 admin_id。
    ...
    /**
     * 是否超级管理员（任一已启用 + is_system=1 + 未软删的角色即视为超管）。
     */
    public function isSuperAdmin(int $userId): bool
    {
        $cacheKey = "user_is_super:{$userId}";
        return TagCache::remember(self::CACHE_TAG, $cacheKey, function () use ($userId) {
    ...
    protected function getRolePermissions(string $role): array
    {
        ...
        return TagCache::remember(self::CACHE_TAG, $cacheKey, function() use ($role) {
