...
class ExceptionHandle extends Handle
{
    ...
    {
        // 仅对 API 请求（JSON / tenantapi / api 路由）返回统一 JSON
    ...
    protected function isApiRequest($request): bool
    {
        ...
        $url = $request->url();
        if (str_starts_with($pathInfo, 'tenantapi/') || str_starts_with($pathInfo, 'platformapi/') || str_starts_with($pathInfo, 'api/')
            || str_contains($url, '/tenantapi/') || str_contains($url, '/platformapi/') || str_contains($url, '/api/')) {
            return true;
        }
