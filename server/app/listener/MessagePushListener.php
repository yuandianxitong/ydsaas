...
class MessagePushListener
{
     ...
     * 根据用户决定推送通道
     *
     * 产品状态：设备画像 / UserDevice 表未落地，当前仅 database 通道；
     * wechat_subscribe 需调用方显式扩通道或后续接设备表后再启用。
     *
     * @return string[] 通道列表
     */
     ...
     * UniPush（个推）推送
     *
     * 产品状态：未接入第三方推送 SDK / 凭据，刻意不实现；
     * 通道名保留以便未来扩展，命中时仅记日志。
     */
    protected function pushToUniPush(array $message): void
    {
        Log::info('MessagePush: uni-push not implemented (product backlog)', [
