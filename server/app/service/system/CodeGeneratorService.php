...
class CodeGeneratorService extends Service
{
    ...
    public function generate(array $config): array
    {
        ...
        // 4. Controller
        $files['controller'] = [
            'path'    => "app/tenantapi/controller/v1/{$moduleName}/{$modelName}Controller.php",
            'content' => $this->generateController($moduleName, $modelName, $tableComment, $columns, $hasStatus),
        ];

        // 5. Validate
        $files['validate'] = [
            'path'    => "app/tenantapi/validate/v1/{$moduleName}/{$modelName}Validate.php",
            'content' => $this->generateValidate($moduleName, $modelName, $columns),
        ];

        // 6. Route
        $files['route'] = [
            'path'    => "app/tenantapi/route/{$moduleName}.php",
        ...
        // 表结构变更不再产出 think-migration 文件；
        // 统一走 database/updates/vX.Y.Z/update.sql + php think yd:update。
    ...
    protected function appendRoute(string $routeFile, string $routeContent): array
    {
        ...
        if (!file_exists($fullPath)) {
            ...
            $newFile = "<?php\nuse think\\facade\\Route;\n\n"
                ...
                . $routeContent . "\n\n"
                . "})->middleware(['tenant_auth', 'admin_permission', 'admin_log']);\n";
...
namespace app\\tenantapi\\controller\\v1\\{$module};

use app\\service\\{$module}\\{$model}Service;
use app\\tenantapi\\validate\\v1\\{$module}\\{$model}Validate;
...
namespace app\\tenantapi\\validate\\v1\\{$module};
...
PHP;
    ...
    updateStatus(id: number, data: { status: number }) {
        return myRequest.put(`/tenantapi/{$module}/{$kebab}/\${id}/status`, data)
    },
TS;
        }
...
export const {$apiName} = {
    ...
    getList(params: Record<string, any>) {
        return myRequest.get('/tenantapi/{$module}/{$kebab}', { params })
    },
     ...
     */
    getDetail(id: number) {
        return myRequest.get(`/tenantapi/{$module}/{$kebab}/\${id}`)
    },
    ...
    create(data: Record<string, any>) {
        return myRequest.post('/tenantapi/{$module}/{$kebab}', data)
    },
    ...
    update(id: number, data: Record<string, any>) {
        return myRequest.put(`/tenantapi/{$module}/{$kebab}/\${id}`, data)
    },
     ...
     */
    delete(id: number) {
        return myRequest.delete(`/tenantapi/{$module}/{$kebab}/\${id}`)
    },
    ...
    batchDelete(data: { ids: number[] }) {
        return myRequest.post('/tenantapi/{$module}/{$kebab}/batch-delete', data)
    },
{$statusApi}}
TS;
    }
