...
class AdminService extends Service
{
    ...
    public function getAdminInfo(int $adminId): array
    {
        ...
        // tenant 端 admins 表没有 is_super 列，超管身份等价于"拥有 is_system=1 的角色"。
        // 沿用 $admin['is_super'] 字段是为了让下游/前端继续以同一个 key 取值。
    ...
    public function createAdmin(array $data): array
    {
        ...
        $admin = $this->runInTransaction(function () use ($adminData, $data) {
    ...
    public function updateAdmin(int $id, array $data): bool
    {
        ...
        $result = $this->runInTransaction(function () use ($id, $updateData, $data) {
    ...
    public function updateStatus(int $id, int $status): bool
    {
        ...
        // 超级管理员不能禁用
        if (!empty($admin['is_super']) && $status == 0) {
